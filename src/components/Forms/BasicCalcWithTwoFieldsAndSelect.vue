<template>

  <div class="container">
    <div class="row">
      <div class="col-lg-20">
        <div class="row row-grid">
          <div class="col-lg-20">
            <!--                              CARD NUMBER 1-->
            <card class="border-0" hover shadow body-classes="py-5">
              <icon name="ni ni-check-bold" type="primary" rounded class="mb-8">
              </icon>
              <h6 class="text-primary text-uppercase">Download Argon</h6>
              <p class="description mt-6">Argon is a great free UI package based on Bootstrap 4
                that includes the most important components and features.</p>
              <div>
                <badge type="primary" rounded>design</badge>
                <badge type="primary" rounded>system</badge>
                <badge type="primary" rounded>creative</badge>
              </div>
              <base-button tag="a" href="#" type="primary" class="mt-4">
                Learn more
              </base-button>
            </card>
          </div>
          </div>
          </div>
          </div>
          </div>


</template>

<script>
export default {
  name: "BasicCalcWithTwoFieldsAndSelect"
}
</script>

<style scoped>

</style>